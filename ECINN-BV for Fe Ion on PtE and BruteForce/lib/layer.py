...
class BVLayer(tf.keras.layers.Layer):
    ...
    def __init__(self,name='FluxLayer'):
        ...
        self.lambda_alpha = tf.Variable(initial_value=0.5,trainable=True,name='lambda_alpha',constraint=tf.keras.constraints.non_neg())
        self.lambda_beta = tf.Variable(initial_value=0.5,trainable=True,name='lambda_beta',constraint=tf.keras.constraints.non_neg())
...
class DiffusionCoefficientLayer(tf.keras.layers.Layer):
    ...
    def __init__(self):
        super().__init__()
        self.lambda_dA = tf.Variable(initial_value=0.5,trainable=True,name ='lambda_d_A',constraint=tf.keras.constraints.non_neg())
