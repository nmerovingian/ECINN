...
# set random seends to increase reproducibility
random.seed(42)
np.random.seed(42)
...
tf.keras.utils.plot_model(ecinn, to_file='ECINN_model.png', rankdir="LR",show_shapes=True,expand_nested=True,dpi=200) #Requires installation of Graphviz and pydot
...
def get_Lambda_values():
    ...
    return lambda_K0_value.numpy(),lambda_alpha_value.numpy(),lambda_dA_value.numpy()
...
class AdaptiveWeightCallBack(tf.keras.callbacks.Callback):
    ...
    def on_epoch_end(self, epoch, logs=None):
        ...
        lambda_K0_value,lambda_alpha_value,lambda_dA_value = get_Lambda_values()
        print('K0',lambda_K0_value,'alpha',lambda_alpha_value,'dA',lambda_dA_value)
    ...
    def on_train_end(self,logs=None):
        logs['BV_weight_1'] = self.BV_weight_1_hist
        logs['BV_weight_2'] = self.BV_weight_2_hist
        logs['BV_weight_3'] = self.BV_weight_3_hist

        logs['lambda_K0'] = self.lambda_K0_value_hist
        logs['lambda_alpha'] = self.lambda_alpha_value_hist
        logs['lambda_dA'] = self.lambda_dA_value_hist

        self.history_dict = logs
...
class DataGenerator(tf.keras.utils.Sequence):
    # The data generator to generate training data on the fly
    def __init__(self,exp_file_names,sigmas,theta_i,theta_v,PortionAnalyzed=0.75,Lambda=3.0,num_train_samples=1000000,batch_size=250):
        super().__init__()
    ...
    def __getitem__(self, index):
        ...
        #Applied potentials and fluxes sampled from experimental data
...
def PINNFitExp(exp_file_names,sigmas,theta_i,theta_v,dA_exp_measured,epochs=200,PortionAnalyzed=0.75,Lambda=3.0,initial_weights=None,train=True,saving_directory='./Data'):
    """
    ...
    def schedule(epoch,lr):
        # a learning rate scheduler, learning rate is decreased by 2% every epoch after the 50th epoch
        if epoch<=50:
            return lr 
        else:
            lr *= 0.98
    ...
    if train:
        ...
        ecinn.fit(training_generator,epochs=epochs,verbose=2,callbacks=[lr_scheduler_callback,adptiveweightcallback],workers=3,use_multiprocessing=False,max_queue_size=24)
        history = adptiveweightcallback.history_dict
    ...
    lambda_k0_eff = lambda_k0*math.exp(lambda_alpha_value*ERef*96485/8.314/298)
    fig_all.suptitle(f'$\lambda_{{K_0}}$={lambda_K0_value:.2E},$\lambda_{{k_0}}$={lambda_k0:.2E}m/s $\lambda_{{k_0,eff}}$={lambda_k0_eff:.2E} m/s\n $\lambda_{{\\alpha}}$ = {lambda_alpha_value:.2f}, $\lambda_{{d_A}}$ = {lambda_dA_value:.3f}, $\lambda_{{D_A}}={lambda_dA_value*1e-9:.2E}m^2s^{{-1}}$',y=1.1)
    ...
    for i in range(len(networks)):
        ...
        #ax.plot([-5,5],[-0.446*math.sqrt(sigma),-0.446*math.sqrt(sigma)],label='R-S Equation',ls='--',color='k',lw=3)
        ax.annotate("", xy=(0.7, -3), xytext=(0.95, -3),arrowprops=dict(facecolor='black', shrink=0.05))
    ...
    fig.text(0.04,0.88,'b)',fontsize=20,fontweight='bold')
    fig.text(0.04,0.48,'c)',fontsize=20,fontweight='bold')
...
if __name__ == "__main__":
    ...
    for epochs in [300]:
        saving_directory = f"Epochs={epochs}"
