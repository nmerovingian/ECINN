...
class NernstLayer(tf.keras.layers.Layer):
    """
    Customized layer for Nernst equilibrium. 
    """
...
class DiffusionCoefficientLayer(tf.keras.layers.Layer):
    ...
    def __init__(self):
        super().__init__()
        self.lambda_dA = tf.Variable(initial_value=0.8,trainable=True,name ='lambda_d_A',constraint=tf.keras.constraints.non_neg())
